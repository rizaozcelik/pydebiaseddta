...
class TFPredictor(BasePredictor):
    @abstractmethod
    ...
    def train(
        ...
        if sample_weights_by_epoch is None:
            sample_weights_by_epoch = create_uniform_weights(
                len(train_chemicals), self.n_epochs
            )
        ...
        if (
            ...
            val_tuple = (
                [val_chemical_vectors, val_protein_vectors],
                np.array(val_labels),
            )
        ...
        for e in range(self.n_epochs):
            self.model.fit(
                x=[train_chemical_vectors, train_protein_vectors],
                y=train_labels,
            ...
            train_stats = evaluate_predictions(
                ...
                predictions=self.predict(train_chemicals, train_proteins),
