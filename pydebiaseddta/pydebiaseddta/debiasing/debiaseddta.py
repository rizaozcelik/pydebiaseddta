...
class DebiasedDTA:
    def __init__(
        ...
        self.predictor_params = (
            {} if predictor_params is None else predictor_params
        )
    ...
    def save_importance_coefficients(
        interactions, importance_coefficients, savedir
    ):
        dump_content = []
    ...
    def learn_importance_coefficients(
        self, train_chemicals, train_proteins, train_labels, savedir=None
    ):
        ...
        for i in range(self.n_bootstrapping):
            ...
            for mini_val_ix in range(n_mini_val):
                ...
                mini_val_interactions = train_interactions[
                    val_start_ix:val_end_ix
                ]
                mini_train_interactions = (
                    train_interactions[:val_start_ix]
                    + train_interactions[val_end_ix:]
                )
                assert (
                    len(mini_train_interactions) + len(mini_val_interactions)
                    == train_size
                )

                mini_train_chemicals = [
                ...
                mini_val_sq_diffs = (
                    np.array(mini_val_labels) - np.array(preds)
                ) ** 2
        ...
        importance_coefficients = [
            med / sum(interaction_id_to_med_diff)
            for med in interaction_id_to_med_diff
        ]
